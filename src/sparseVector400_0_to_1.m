...
figure
ax1 = subplot(1, 2, 1);
plot(ax1, ms, costs0, '-ro', ms, costs1,'-md', ms, costst, '-k+', ms, costs2, '-bx', 'LineWidth', 1.5);
title(ax1, 'Cost vs. sample size (1-norm)');
xlabel(ax1, 'Sample size (m)');
ylabel(ax1, 'Cost');

ax2 = subplot(1, 2, 2);
plot(ax2, ms, risks0, '-ro', ms, risks1, '-md', ms, riskst, '-k+', ms, risks2, '-bx', 'LineWidth', 1.5);
title(ax2,'Risk vs. sample size (1-norm)')
xlabel(ax2, 'Sample size (m)');
ylabel(ax2,'Risk')

h = legend('Constant smooth', 'Constant risk', 'Alpha 0.8', 'Alpha 0.9');
set(h, 'FontSize', 15);
